...
Ships alongside a single module file (``toon_fire.py``). Two ways to
...
* Fetches ``toon_fire.py`` fresh from GitHub (SHA-pinned URL, so no CDN
...
* Wipes any ``__pycache__/toon_fire.cpython-*.pyc`` so stale bytecode
  doesn't shadow the freshly copied source.
* Flushes ``toon_fire`` from ``sys.modules`` so the next import reads
...
_MODULE = "toon_fire"                    # your tool's .py filename (without .py)
_SHELF_BUTTON_LABEL = "ToonFire"         # short label on the shelf button
# --- END CUSTOMIZE -------------------------------------------------------
